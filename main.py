...
title = pygame.image.load('assets/images/menu/title.png')
...
def color_calc(player):
	if 255 + 255 - player.dmg_taken <= 0:
		...
		return (255+255-player.dmg_taken, 0, 0, 150)
	else:
		return (255, 255 - player.dmg_taken, 255 - player.dmg_taken, min(player.dmg_taken*2,150))
...
def draw_menu(win, time_val, mouse_pos, mouse_click, screen):
	...
	if time_val < 120:
		...
		win.blit(title, (WIDTH*1//6, 30))
	if start_rect.collidepoint(mouse_pos):
		if button_hover_last == False:
...
def main():
	...
	left_player=Player(WIDTH*2//5, HEIGHT//2 - PADDLE_HEIGHT//2, PADDLE_WIDTH*2, PADDLE_HEIGHT*2, 2.5, pygame.K_d, pygame.K_a, pygame.K_w, pygame.K_s, pygame.K_f, pygame.K_g, pygame.K_v, False)
	right_player=Player(WIDTH*3//5, HEIGHT//2 - PADDLE_HEIGHT//2, PADDLE_WIDTH*2, PADDLE_HEIGHT*2, 2.5, pygame.K_RIGHT, pygame.K_LEFT, pygame.K_UP, pygame.K_DOWN, pygame.K_j, pygame.K_k, pygame.K_m, True)
	...
	while run:
		...
		if screen[0] == 0:
			...
			if screen_0_switch:
				...
				screen_1_switch = True
			draw_menu(WIN, time_val, mouse_pos, mouseClick, screen)
		...
		if screen[0] == 1:
			...
			if right_player.stocks == 0:
				won = True
				WIN.blit(goku_wins, (WIDTH//2 - goku_wins.get_width()//2, HEIGHT//2 - goku_wins.get_height()//2))

			if left_player.stocks == 0:
				won = True
				WIN.blit(vegeta_wins, (WIDTH//2 - vegeta_wins.get_width()//2, HEIGHT//2 - vegeta_wins.get_height()//2))
			if won:
